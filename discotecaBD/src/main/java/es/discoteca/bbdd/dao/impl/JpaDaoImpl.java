...
import es.discoteca.bbdd.dao.JpaDao;

public abstract class JpaDaoImpl<K, E> extends JpaDaoSupport implements
		JpaDao<K, E> {
	...
	public List<E> findAllPaginate(final int first, final int pageSize,
		...
		Object res = getJpaTemplate().execute(new JpaCallback() {
			...
			public Object doInJpa(final EntityManager em)
					throws PersistenceException {
				Query q = em.createQuery("SELECT h FROM "
						+ entityClass.getName() + " h ORDER BY " + orderBy
						+ " ASC");
